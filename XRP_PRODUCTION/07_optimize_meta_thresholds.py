...
def optimize():
    logger.info(f"\n{'='*70}")
    logger.info(f"XRP META THRESHOLD OPTIMIZER")
    logger.info(f"{'='*70}\n")

    # Load everything
    long_model, long_seq, long_ckpt = load_cnn(LONG_MODEL_DIR, 'XRP_direction_model.pt')
    short_model, short_seq, short_ckpt = load_cnn(SHORT_MODEL_DIR, 'XRP_short_model.pt')
    long_temp = long_ckpt.get('temperature', 1.0)
    short_temp = short_ckpt.get('temperature', 1.0)

    meta_long = joblib.load(META_MODEL_DIR / 'XRP_meta_long.joblib')
    meta_short = joblib.load(META_MODEL_DIR / 'XRP_meta_short.joblib')
    ...
    # Load 15min data from the full download
    df_15m = pd.read_csv(DATA_DIR / 'xrp_15m_data.csv')
    df_15m['timestamp'] = pd.to_datetime(df_15m['timestamp'], unit='ms')
    df_15m = df_15m[(df_15m['timestamp'] >= '2025-12-15') & (df_15m['timestamp'] <= '2026-04-30')]
