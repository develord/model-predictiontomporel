...
def fetch_15min():
    ...
    cur = s
    while cur < e:
        c = ex.fetch_ohlcv('DOT/USDT', '15m', since=cur, limit=1000)
        if not c: break
        candles.extend(c)
        cur = c[-1][0] + 1
...
def get_tp_sl(row, entry, direction, s_tp=0.020, s_sl=0.010):
    ...
    if direction == 'LONG':
        if USE_DYNAMIC_TP_SL and atr and pd.notna(atr) and atr > 0:
            tp_m = min(max(atr / entry, 0.008), 0.025)  # V2: cap 2.5% (was 3%)
            sl_m = tp_m * 0.65                            # V2: wider SL (was 0.5)
        else:
            tp_m, sl_m = TP_PCT, SL_PCT
        return entry * (1 + tp_m), entry * (1 - sl_m)
    else:
        # SHORT: fixed 3% TP / 3% SL (V2 best)
...
def check_long_filters(row, consec, cool, date):
    ...
    # V2: Weekly momentum filter - block LONG if weekly trend is bearish
    if '1w_momentum_5' in row.index and pd.notna(row['1w_momentum_5']):
        if row['1w_momentum_5'] < -0.10:  # Weekly momentum strongly negative
...
def backtest():
    logger.info(f"\n{'='*70}")
    logger.info(f"DOT INDEPENDENT LONG + SHORT BACKTEST (Q1 2026)")
    logger.info(f"{'='*70}\n")

    # Load models
    long_model, long_seq, long_ckpt = load_model(LONG_MODEL_DIR, 'dot_cnn_model.pt')
    short_model, short_seq, short_ckpt = load_model(SHORT_MODEL_DIR, 'DOT_short_model.pt')
    if not long_model or not short_model:
        logger.error("Missing model(s)")
        return

    # LONG features
    with open(LONG_MODEL_DIR / 'dot_cnn_features.json') as f:
    ...
    df = pd.read_csv(DATA_DIR / 'dot_multi_tf_merged.csv')
    ...
    if trades:
        ...
        tdf.to_csv(RESULTS_DIR / 'dot_independent_backtest.csv', index=False)
    else:
