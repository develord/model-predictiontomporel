...
def optimize():
    logger.info(f"\n{'='*70}")
    logger.info(f"FIL META THRESHOLD OPTIMIZER")
    logger.info(f"{'='*70}\n")

    # Load everything
    long_model, long_seq, long_ckpt = load_cnn(LONG_MODEL_DIR, 'FIL_direction_model.pt')
    short_model, short_seq, short_ckpt = load_cnn(SHORT_MODEL_DIR, 'FIL_short_model.pt')
    long_temp = long_ckpt.get('temperature', 1.0)
    short_temp = short_ckpt.get('temperature', 1.0)

    meta_long = joblib.load(META_MODEL_DIR / 'FIL_meta_long.joblib')
    meta_short = joblib.load(META_MODEL_DIR / 'FIL_meta_short.joblib')
    ...
    # Load 15min
    cache = DATA_DIR / 'fil_15min_q1_2026.csv'
