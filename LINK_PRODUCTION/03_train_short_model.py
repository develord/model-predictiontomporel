"""
LINK SHORT CNN Training Script V3
==================================
Improved SHORT model with:
- ATR-based symmetric TP/SL labels
- Bear-specific features added to existing features
- DeepCNNShortModel (conv 3,5,9 + 3 blocks)
...
GRAD_CLIP = 1.0
NOISE_STD = 0.005
LABEL_SMOOTHING = 0.0  # No smoothing to force learning
...
def train():
    logger.info(f"\n{'='*70}")
    logger.info(f"LINK SHORT CNN V3 TRAINING (DeepCNN + Bear Features, seq=45)")
    logger.info(f"{'='*70}\n")
    ...
    # Scale - clip extreme raw values first to prevent scaler issues
    X_train_raw = np.clip(train_df[feature_cols].fillna(0).replace([np.inf, -np.inf], 0).values.astype(np.float32), -1e6, 1e6)
    X_val_raw = np.clip(val_df[feature_cols].fillna(0).replace([np.inf, -np.inf], 0).values.astype(np.float32), -1e6, 1e6)
    ...
    # Class weights (strongly favor SHORT class to prevent collapse)
    ...
    model = DeepCNNShortModel(feature_dim=FEATURE_DIM, sequence_length=SEQUENCE_LENGTH, dropout=0.30).to(device)
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=20, min_lr=1e-5)

    patience_cnt, best_epoch = 0, 0
    # Always save initial model to ensure checkpoint exists (will be overwritten)
    torch.save({
        ...
        'epoch': 0,
        'val_acc': 0.0,
    }, MODEL_DIR / 'LINK_short_model.pt')
    best_acc = -1.0  # Force first valid save to win
    ...
    for epoch in range(EPOCHS):
        ...
        for xb, yb in train_loader:
            ...
            if torch.isnan(out).any():
                # Reset model to last good state if NaN
                if best_epoch > 0:
                    ckpt = torch.load(MODEL_DIR / 'LINK_short_model.pt')
        ...
        # Save if better balanced accuracy (require both classes predicted)
        if n_short >= 3 and n_no >= 3:
            short_prec = la[pa == 1].mean() if n_short > 0 else 0
            no_prec = (1 - la[pa == 0]).mean() if n_no > 0 else 0
            balanced_score = (short_prec + no_prec) / 2
        else:
            balanced_score = 0.0

        scheduler.step(balanced_score)

        if (epoch + 1) % 10 == 0:
            logger.info(f"E{epoch+1:3d}/{EPOCHS} | Acc:{acc:.3f} | SHORT={n_short} NO={n_no} | ShortPrec={sp:.0f}% | BS:{balanced_score:.3f}")

        if balanced_score > best_acc:
    ...
    ckpt_data = torch.load(MODEL_DIR / 'LINK_short_model.pt')
    ckpt_data['temperature'] = optimal_temp
    torch.save(ckpt_data, MODEL_DIR / 'LINK_short_model.pt')
    ...
    logger.info(f"\nModel: {(MODEL_DIR / 'LINK_short_model.pt').stat().st_size / 1024:.0f} KB")
