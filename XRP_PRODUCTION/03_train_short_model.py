"""
XRP SHORT CNN Training Script V2
==================================
...
def train():
    logger.info(f"\n{'='*70}")
    logger.info(f"XRP SHORT CNN V2 TRAINING (DeepCNN + Bear Features)")
    logger.info(f"{'='*70}\n")
    ...
    logger.info(f"\nModel: {(MODEL_DIR / 'XRP_short_model.pt').stat().st_size / 1024:.0f} KB")
