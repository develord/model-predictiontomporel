...
def optimize():
    logger.info(f"\n{'='*70}")
    logger.info(f"AVAX META THRESHOLD OPTIMIZER")
    logger.info(f"{'='*70}\n")

    # Load everything
    long_model, long_seq, long_ckpt = load_cnn(LONG_MODEL_DIR, 'AVAX_direction_model.pt')
    short_model, short_seq, short_ckpt = load_cnn(SHORT_MODEL_DIR, 'AVAX_short_model.pt')
