...
def train_meta():
    logger.info(f"\n{'='*70}")
    logger.info(f"ADA META-MODEL XGBOOST TRAINING")
    logger.info(f"{'='*70}\n")

    # Load CNN models
    long_model, long_seq, long_ckpt = load_cnn_model(LONG_MODEL_DIR, 'ADA_direction_model.pt')
    short_model, short_seq, short_ckpt = load_cnn_model(SHORT_MODEL_DIR, 'ADA_short_model.pt')
    ...
    if len(lt) > 50:
        ...
        joblib.dump(xgb_long, META_MODEL_DIR / 'ADA_meta_long.joblib')
        logger.info(f"\n  Saved: ADA_meta_long.joblib")
    else:
    ...
    if len(st) > 50:
        ...
        joblib.dump(xgb_short, META_MODEL_DIR / 'ADA_meta_short.joblib')
        logger.info(f"\n  Saved: ADA_meta_short.joblib")
    else:
