...
def load_cnn(model_dir, model_file):
    ...
    feat_dim = ckpt.get('feature_dim', 126)
...
def optimize():
    logger.info(f"\n{'='*70}")
    logger.info(f"NEAR META THRESHOLD OPTIMIZER")
    logger.info(f"{'='*70}\n")

    # Load everything
    long_model, long_seq, long_ckpt = load_cnn(LONG_MODEL_DIR, 'NEAR_direction_model.pt')
    short_model, short_seq, short_ckpt = load_cnn(SHORT_MODEL_DIR, 'NEAR_short_model.pt')
    long_temp = long_ckpt.get('temperature', 1.0)
    short_temp = short_ckpt.get('temperature', 1.0)

    meta_long = joblib.load(META_MODEL_DIR / 'NEAR_meta_long.joblib')
    meta_short = joblib.load(META_MODEL_DIR / 'NEAR_meta_short.joblib')
    ...
    # Load 15min
    cache = DATA_DIR / 'near_15min_q1_2026.csv'
    ...
    if len(rdf_balanced) == 0:
        logger.info("  No configs meet WR >= 55% AND Return > 0% criteria")
        logger.info(f"\n  Relaxed: WR >= 50% AND Return > 0%")
        rdf_rel = rdf_valid[(rdf_valid['wr'] >= 50) & (rdf_valid['return'] > 0)].sort_values('return', ascending=False)
        for idx, row in rdf_rel.head(5).iterrows():
            meta_str = ""
            if row['long_meta'] > 0:
                meta_str += f"LMeta>={row['long_meta']:.0%} "
            if row['short_meta'] > 0:
