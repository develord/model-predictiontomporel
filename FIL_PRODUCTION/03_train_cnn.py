...
def train():
    logger.info(f"\n{'='*70}")
    logger.info(f"FIL 1D-CNN TRAINING (Multi-scale Conv + Attention)")
    logger.info(f"{'='*70}\n")
    ...
    for epoch in range(EPOCHS):
        ...
        if is_better:
            ...
            torch.save({
                ...
                'model_type': 'cnn'
            }, MODEL_DIR / 'FIL_direction_model.pt')
    ...
    # Load best
    ckpt = torch.load(MODEL_DIR / 'FIL_direction_model.pt')
    ...
    torch.save(ckpt, MODEL_DIR / 'FIL_direction_model.pt')
    ...
    logger.info(f"\nModel size: {(MODEL_DIR / 'FIL_direction_model.pt').stat().st_size / 1024:.0f} KB")
