...
- Short sequences (20 days) = more training samples
...
def train():
    logger.info(f"\n{'='*70}")
    logger.info(f"XRP 1D-CNN TRAINING (Multi-scale Conv + Attention)")
    logger.info(f"{'='*70}\n")
    ...
    # Scale features with RobustScaler (handles outliers better than StandardScaler)
    ...
    for epoch in range(EPOCHS):
        ...
        # Compute BUY precision (what matters for trading)
        buy_mask = preds == 1
        ...
        # Save best model based on val accuracy (with diversity requirement)
        ...
        if is_better:
            ...
            torch.save({
                ...
                'model_type': 'cnn'
            }, MODEL_DIR / 'XRP_direction_model.pt')
    ...
    # Load best
    ckpt = torch.load(MODEL_DIR / 'XRP_direction_model.pt')
    ...
    torch.save(ckpt, MODEL_DIR / 'XRP_direction_model.pt')
    ...
    # Clean old files
    for old in ['XRP_direction_model_legacy.pt', 'xrp_xgboost.joblib']:
        p = MODEL_DIR / old
        if p.exists():
            p.unlink()

    logger.info(f"\nModel size: {(MODEL_DIR / 'XRP_direction_model.pt').stat().st_size / 1024:.0f} KB")
