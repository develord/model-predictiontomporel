"""
BTC Meta Threshold Optimizer
==============================
Grid search sur les thresholds CNN + Meta pour maximiser le win rate.
...
def precompute_trade_outcomes(predictions, df_15m, df_test, short_tp, short_sl,
    ...
    Returns list of dicts with all info needed for fast grid search."""
    outcomes = []
    for i in range(len(df_test)):
        pred = predictions[i]
        if pred is None:
...
def optimize():
    logger.info(f"\n{'='*70}")
    logger.info(f"BTC META THRESHOLD OPTIMIZER")
    logger.info(f"{'='*70}\n")

    # Load everything
    long_model, long_seq, long_ckpt = load_cnn(LONG_MODEL_DIR, 'BTC_direction_model.pt')
    short_model, short_seq, short_ckpt = load_cnn(SHORT_MODEL_DIR, 'BTC_short_model.pt')
    long_temp = long_ckpt.get('temperature', 1.0)
    short_temp = short_ckpt.get('temperature', 1.0)

    meta_long = joblib.load(META_MODEL_DIR / 'BTC_meta_long.joblib')
    meta_short = joblib.load(META_MODEL_DIR / 'BTC_meta_short.joblib')
    ...
    # Save
    rdf.to_csv(BASE_DIR / 'results' / 'threshold_optimization.csv', index=False)
