...
def run_backtest(use_meta=True, label=""):
    ...
    if use_meta:
        meta_long_path = META_MODEL_DIR / 'BTC_meta_long.joblib'
        meta_short_path = META_MODEL_DIR / 'BTC_meta_short.joblib'
...
def backtest():
    logger.info(f"\n{'='*70}")
    logger.info(f"BTC META-MODEL BACKTEST COMPARISON")
    logger.info(f"{'='*70}\n")
    ...
    for label, capital, trades in [("CNN+Meta", cap_meta, trades_meta), ("CNN-only", cap_cnn, trades_cnn)]:
        if trades:
            ...
            tdf.to_csv(RESULTS_DIR / f'btc_backtest_{label.lower().replace("+", "_")}.csv', index=False)
        else:
