...
def train_meta():
    logger.info(f"\n{'='*70}")
    logger.info(f"ETH META-MODEL XGBOOST TRAINING")
    logger.info(f"{'='*70}\n")

    # Load CNN models
    long_model, long_seq, long_ckpt = load_cnn_model(LONG_MODEL_DIR, 'ETH_direction_model.pt')
    short_model, short_seq, short_ckpt = load_cnn_model(SHORT_MODEL_DIR, 'ETH_short_model.pt')
