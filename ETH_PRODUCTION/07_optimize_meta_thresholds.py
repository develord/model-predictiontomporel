...
def optimize():
    logger.info(f"\n{'='*70}")
    logger.info(f"ETH META THRESHOLD OPTIMIZER")
    logger.info(f"{'='*70}\n")
    ...
    meta_long = joblib.load(META_MODEL_DIR / 'ETH_meta_long.joblib')
    meta_short = joblib.load(META_MODEL_DIR / 'ETH_meta_short.joblib')
    ...
    # Save
    rdf.to_csv(BASE_DIR / 'results' / 'eth_threshold_optimization.csv', index=False)
    logger.info(f"\nAll results saved to results/eth_threshold_optimization.csv")
