"""
BTC Production Retraining Script
==================================
Retrains ALL BTC models on the FULL dataset (no train/val split) for
production deployment:
  1. LONG CNN  (CNNDirectionModel)
  2. SHORT CNN (CNNDirectionModel)
  3. Meta XGBoost LONG
  4. Meta XGBoost SHORT

Key principles:
  - Train on ALL available data (no validation holdout)
  - Use FIXED number of epochs (not early stopping, since no val set)
  - Keep temperature values from previous calibrated runs (already optimized)
...
# LONG CNN params
# ============================================================
LONG_FIXED_EPOCHS = 8      # Previous best ~epoch 5, 1.5x buffer
LONG_BATCH_SIZE = 64
LONG_LR = 0.0015
LONG_NOISE_STD = 0.015
LONG_LABEL_SMOOTHING = 0.1
LONG_AUGMENT_COPIES = 2    # 3x total (original + 2 copies)
LONG_TEMPERATURE = 1.692   # From previous calibrated run

# ============================================================
# SHORT CNN params
# ============================================================
SHORT_FIXED_EPOCHS = 50    # Previous best ~epoch 35, buffer
SHORT_BATCH_SIZE = 64
SHORT_LR = 0.0005
SHORT_NOISE_STD = 0.01
SHORT_LABEL_SMOOTHING = 0.05
SHORT_AUGMENT_COPIES = 3   # 4x total
SHORT_TEMPERATURE = 2.165  # From previous calibrated run
...
def train_long_cnn():
    ...
    }, LONG_MODEL_DIR / 'BTC_direction_model.pt')

    size_kb = (LONG_MODEL_DIR / 'BTC_direction_model.pt').stat().st_size / 1024
    logger.info(f"Saved: {LONG_MODEL_DIR / 'BTC_direction_model.pt'} ({size_kb:.0f} KB)")
...
# ============================================================
# STEP 2: Train SHORT CNN on full data
# ============================================================
def train_short_cnn():
    logger.info(f"\n{'='*70}")
    ...
    for epoch in range(SHORT_FIXED_EPOCHS):
        ...
        scheduler.step(epoch)
        if (epoch + 1) % 10 == 0:
            logger.info(f"  E{epoch+1:3d}/{SHORT_FIXED_EPOCHS} | Loss: {train_loss:.4f} | Acc: {train_acc:.3f} | "
                         f"LR: {optimizer.param_groups[0]['lr']:.6f}")
    ...
    }, SHORT_MODEL_DIR / 'BTC_short_model.pt')

    # Save feature list
    with open(SHORT_MODEL_DIR / 'short_features.json', 'w') as f:
        json.dump(feature_cols, f)

    size_kb = (SHORT_MODEL_DIR / 'BTC_short_model.pt').stat().st_size / 1024
    logger.info(f"Saved: {SHORT_MODEL_DIR / 'BTC_short_model.pt'} ({size_kb:.0f} KB)")
...
# ============================================================
# STEP 3: Train Meta XGBoost (LONG + SHORT) on full data
# ============================================================
def train_meta_xgboost():
    logger.info(f"\n{'='*70}")
    logger.info(f"STEP 3: META XGBOOST - Production Retrain (ALL data, {META_N_ESTIMATORS} trees)")
    logger.info(f"{'='*70}\n")

    # Load the just-trained CNN models
    long_ckpt = torch.load(LONG_MODEL_DIR / 'BTC_direction_model.pt', map_location='cpu', weights_only=False)
    ...
    short_ckpt = torch.load(SHORT_MODEL_DIR / 'BTC_short_model.pt', map_location='cpu', weights_only=False)
    short_model = CNNDirectionModel(
    ...
    for i in range(seq, len(df)):
        ...
        # LONG prediction
        lx = torch.tensor(long_feat[i - seq:i], dtype=torch.float32).unsqueeze(0)
        with torch.no_grad():
        ...
        # SHORT prediction
        sx = torch.tensor(short_feat[i - seq:i], dtype=torch.float32).unsqueeze(0)
        with torch.no_grad():
    ...
    if len(lt) > 50:
        ...
        joblib.dump(xgb_long, META_MODEL_DIR / 'BTC_meta_long.joblib')
        logger.info(f"  Saved: {META_MODEL_DIR / 'BTC_meta_long.joblib'}")
    ...
    if len(st) > 50:
        ...
        joblib.dump(xgb_short, META_MODEL_DIR / 'BTC_meta_short.joblib')
        logger.info(f"  Saved: {META_MODEL_DIR / 'BTC_meta_short.joblib'}")
        ...
        logger.warning(f"  Not enough SHORT samples ({len(st)}), skipping")
...
def main():
    logger.info(f"\n{'#'*70}")
    logger.info(f"#  BTC PRODUCTION RETRAIN - ALL MODELS ON FULL DATASET")
    ...
    # Step 3: Meta XGBoost (uses the just-trained CNN models)
    train_meta_xgboost()

    logger.info(f"\n{'#'*70}")
    logger.info(f"#  PRODUCTION RETRAIN COMPLETE")
    logger.info(f"{'#'*70}")
    logger.info(f"  LONG CNN:       {LONG_MODEL_DIR / 'BTC_direction_model.pt'}")
    logger.info(f"  LONG scaler:    {LONG_MODEL_DIR / 'feature_scaler.joblib'}")
    logger.info(f"  SHORT CNN:      {SHORT_MODEL_DIR / 'BTC_short_model.pt'}")
    ...
    logger.info(f"  Meta LONG:      {META_MODEL_DIR / 'BTC_meta_long.joblib'}")
    logger.info(f"  Meta SHORT:     {META_MODEL_DIR / 'BTC_meta_short.joblib'}")
    ...
    logger.info(f"  Meta trees: {META_N_ESTIMATORS} (no early stopping)")
