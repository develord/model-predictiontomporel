...
def fetch_15min():
    ...
    cur = s
    while cur < e:
        c = ex.fetch_ohlcv('SOL/USDT', '15m', since=cur, limit=1000)
        if not c: break
        candles.extend(c)
        cur = c[-1][0] + 1
...
def backtest():
    logger.info(f"\n{'='*70}")
    logger.info(f"SOL INDEPENDENT LONG + SHORT BACKTEST (Q1 2026)")
    logger.info(f"{'='*70}\n")

    # Load models
    long_model, long_seq, long_ckpt = load_model(LONG_MODEL_DIR, 'SOL_direction_model.pt')
    short_model, short_seq, short_ckpt = load_model(SHORT_MODEL_DIR, 'SOL_short_model.pt')
    ...
    # Temperature for calibrated confidence (same as signal_generator.py)
    ...
    if trades:
        ...
        tdf.to_csv(RESULTS_DIR / 'sol_independent_backtest.csv', index=False)
    else:
