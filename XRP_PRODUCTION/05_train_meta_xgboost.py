...
def train_meta():
    logger.info(f"\n{'='*70}")
    logger.info(f"XRP META-MODEL XGBOOST TRAINING")
    logger.info(f"{'='*70}\n")

    # Load CNN models
    long_model, long_seq, long_ckpt = load_cnn_model(LONG_MODEL_DIR, 'XRP_direction_model.pt')
    short_model, short_seq, short_ckpt = load_cnn_model(SHORT_MODEL_DIR, 'XRP_short_model.pt')
    ...
    if len(st) > 50:
        ...
        joblib.dump(xgb_short, META_MODEL_DIR / 'XRP_meta_short.joblib')
        logger.info(f"\n  Saved: XRP_meta_short.joblib")
    else:
