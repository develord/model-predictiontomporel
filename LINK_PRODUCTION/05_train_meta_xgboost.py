...
def train_meta():
    logger.info(f"\n{'='*70}")
    logger.info(f"{CRYPTO} META-MODEL XGBOOST TRAINING")
    logger.info(f"{'='*70}\n")

    # Load CNN models
    long_model, long_seq, long_ckpt = load_cnn_model(LONG_MODEL_DIR, f'{CRYPTO}_direction_model.pt')
    short_model, short_seq, short_ckpt = load_cnn_model(SHORT_MODEL_DIR, f'{CRYPTO}_short_model.pt')
    ...
    if len(lt) > 50:
        ...
        logger.info(f"\n  Saved: {CRYPTO}_meta_long.joblib")
    else:
    ...
    if len(st) > 50:
        ...
        logger.info(f"\n  Saved: {CRYPTO}_meta_short.joblib")
    else:
