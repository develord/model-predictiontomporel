"""
LINK Production Retraining Script
==================================
Retrains LINK models on the FULL dataset (no train/val split) for
production deployment:
  1. LONG CNN  (CNNDirectionModel)
  2. SHORT CNN (DeepCNNShortModel - LayerNorm variant)
...
Key principles:
  ...
  - LINK SHORT uses LayerNorm architecture (BatchNorm caused NaN explosions)
...
# ============================================================
# DeepCNNShortModel (LayerNorm variant - stable for LINK)
# ============================================================
class DeepCNNShortModel(nn.Module):
    """CNN for SHORT detection - simplified architecture for stable training.
    Uses LayerNorm instead of BatchNorm and simpler 2-block design.
    This is the EXACT architecture from LINK 03_train_short_model.py that
    succeeded in training (BatchNorm version caused NaN explosions)."""

    def __init__(self, feature_dim, sequence_length=45, dropout=0.35):
        super().__init__()
        self.sequence_length = sequence_length

        # Feature projection with LayerNorm (more stable than BatchNorm)
...
# ============================================================
LONG_FIXED_EPOCHS = 5      # Best epoch from calibrated run (checkpoint epoch=5)
LONG_BATCH_SIZE = 64
LONG_LR = 0.0015
LONG_NOISE_STD = 0.015
LONG_LABEL_SMOOTHING = 0.1
LONG_AUGMENT_COPIES = 2    # 3x total (original + 2 copies)
LONG_TEMPERATURE = 1.4718  # From LINK calibrated run

# ============================================================
# SHORT CNN params (from 03_train_short_model.py calibrated run)
# ============================================================
SHORT_FIXED_EPOCHS = 9     # Best epoch from calibrated run (checkpoint epoch=9)
SHORT_BATCH_SIZE = 32
SHORT_LR = 0.001
SHORT_NOISE_STD = 0.005
SHORT_LABEL_SMOOTHING = 0.0   # No smoothing to force learning
SHORT_AUGMENT_COPIES = 3      # 4x total
SHORT_TEMPERATURE = 2.2645    # From LINK calibrated run
...
def train_epoch_simple(model, dataloader, criterion, optimizer, device, grad_clip):
    ...
    for X_b, y_b in dataloader:
        ...
        if torch.isnan(out).any():
            continue  # Skip NaN batches (safety for LINK SHORT)
...
def train_long_cnn():
    ...
    }, LONG_MODEL_DIR / 'LINK_direction_model.pt')

    size_kb = (LONG_MODEL_DIR / 'LINK_direction_model.pt').stat().st_size / 1024
    logger.info(f"Saved: {LONG_MODEL_DIR / 'LINK_direction_model.pt'} ({size_kb:.0f} KB)")
...
# ============================================================
# STEP 2: Train SHORT CNN on full data (DeepCNNShortModel - LayerNorm)
# ============================================================
def train_short_cnn():
    logger.info(f"\n{'='*70}")
    ...
    # Scale ALL data - clip extreme raw values first to prevent scaler issues
    X_raw = np.clip(
        df[feature_cols].fillna(0).replace([np.inf, -np.inf], 0).values.astype(np.float32),
        -1e6, 1e6
    )
    scaler = RobustScaler()
    X_scaled = np.clip(
    ...
    # Class weights (extra 2x boost for SHORT class - matches LINK training)
    n_total = len(train_labels)
    w0 = n_total / (2 * n_0) if n_0 > 0 else 1.0
    w1 = (n_total / (2 * n_1)) * 2.0 if n_1 > 0 else 1.0  # Extra 2x to force SHORT learning
    logger.info(f"Class weights: NO={w0:.3f}, SHORT={w1:.3f}")
    ...
    # Model - DeepCNNShortModel (LayerNorm variant) for LINK SHORT
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = DeepCNNShortModel(feature_dim=FEATURE_DIM, sequence_length=SHORT_SEQUENCE_LENGTH, dropout=0.30).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    logger.info(f"Model: DeepCNNShortModel (LayerNorm, SHORT) | Params: {n_params:,} | Device: {device}")

    criterion = nn.CrossEntropyLoss(
        weight=torch.FloatTensor([w0, w1]).to(device),
        label_smoothing=SHORT_LABEL_SMOOTHING
    )
    optimizer = optim.AdamW(model.parameters(), lr=SHORT_LR, weight_decay=1e-4)
    ...
    }, SHORT_MODEL_DIR / 'LINK_short_model.pt')

    # Save feature list
    with open(SHORT_MODEL_DIR / 'short_features.json', 'w') as f:
        json.dump(feature_cols, f)

    size_kb = (SHORT_MODEL_DIR / 'LINK_short_model.pt').stat().st_size / 1024
    logger.info(f"Saved: {SHORT_MODEL_DIR / 'LINK_short_model.pt'} ({size_kb:.0f} KB)")
...
def train_meta_xgboost():
    """Retrain meta XGBoost models on ALL data (no train/val split)."""
    ...
    # Load freshly trained CNN models
    long_ckpt = torch.load(LONG_MODEL_DIR / 'LINK_direction_model.pt', map_location='cpu', weights_only=False)
    ...
    short_ckpt = torch.load(SHORT_MODEL_DIR / 'LINK_short_model.pt', map_location='cpu', weights_only=False)
    ...
    logger.info(f"Temperature: LONG={long_temp:.4f}, SHORT={short_temp:.4f}")
    ...
    long_raw = np.clip(df[long_feature_cols].fillna(0).replace([np.inf, -np.inf], 0).values.astype(np.float32), -1e6, 1e6)
    ...
    for c in short_feature_cols:
        if c not in df.columns:
            df[c] = 0
    short_raw = np.clip(df[short_feature_cols].fillna(0).replace([np.inf, -np.inf], 0).values.astype(np.float32), -1e6, 1e6)
    short_feat = np.clip(np.nan_to_num(short_scaler.transform(short_raw), nan=0, posinf=0, neginf=0), -5, 5)

    seq = max(long_seq, short_seq)

    # Generate predictions + outcomes
    ...
    for i in range(seq, len(df)):
        ...
        # Simulate outcomes
        long_outcome = _simulate_long_outcome(df, i, atr_val)
        short_outcome = _simulate_short_outcome(df, i, atr_val)

        # Build meta features
        feat = {
            'long_conf': l_conf, 'long_dir': l_dir,
            'short_conf': s_conf, 'short_dir': s_dir,
            'long_prob_spread': l_p1 - l_p0,
            'short_prob_spread': s_p1 - s_p0,
            'models_agree_bull': int(l_dir == 1 and s_dir == 0),
            'models_agree_bear': int(l_dir == 0 and s_dir == 1),
            'models_conflict': int(l_dir == 1 and s_dir == 1),
            'models_neutral': int(l_dir == 0 and s_dir == 0),
            'conf_diff': l_conf - s_conf,
            'max_conf': max(l_conf, s_conf),
            'min_conf': min(l_conf, s_conf),
        }
        for col in market_cols:
            val = row.get(col, np.nan) if col in row.index else np.nan
            feat[col] = val if pd.notna(val) else 0.0

        feat['long_outcome'] = long_outcome
        feat['short_outcome'] = short_outcome
        meta_rows.append(feat)

    meta_df = pd.DataFrame(meta_rows)
    feature_cols_meta = [c for c in meta_df.columns if c not in ['long_outcome', 'short_outcome']]

    logger.info(f"Generated {len(meta_df)} prediction rows, {len(feature_cols_meta)} meta features")
    ...
    if len(lt) > 50:
        ...
        n_win = (y_lt == 1).sum()
        n_lose = (y_lt == 0).sum()
        logger.info(f"  Win={n_win}, Lose={n_lose}")

        xgb_long = XGBClassifier(
            n_estimators=300, max_depth=4, learning_rate=0.05,
            subsample=0.8, colsample_bytree=0.7, min_child_weight=5,
            gamma=1, reg_alpha=0.5, reg_lambda=2,
            scale_pos_weight=n_lose / max(n_win, 1),
            random_state=42, verbosity=0,
        )
        xgb_long.fit(X_lt, y_lt)
        joblib.dump(xgb_long, META_MODEL_DIR / 'LINK_meta_long.joblib')
        logger.info(f"  Saved: LINK_meta_long.joblib")
    ...
    if len(st) > 50:
        ...
        joblib.dump(xgb_short, META_MODEL_DIR / 'LINK_meta_short.joblib')
        logger.info(f"  Saved: LINK_meta_short.joblib")

    # Save meta feature list
    with open(META_MODEL_DIR / 'meta_features.json', 'w') as f:
        json.dump(feature_cols_meta, f, indent=2)

    logger.info(f"  Meta features saved: {META_MODEL_DIR / 'meta_features.json'}")


def _simulate_long_outcome(df, i, atr_val):
    if i >= len(df) - 2:
        return -1
    entry = df.iloc[i]['close']
...
def _simulate_short_outcome(df, i, atr_val):
    if i >= len(df) - 2:
        return -1
    entry = df.iloc[i]['close']
    if pd.notna(atr_val) and atr_val > 0:
...
def main():
    logger.info(f"\n{'#'*70}")
    logger.info(f"#  LINK PRODUCTION RETRAIN - CNN + META MODELS ON FULL DATASET")
    ...
    # Step 2: SHORT CNN (LayerNorm variant)
    ...
    logger.info(f"{'#'*70}")
    logger.info(f"  LONG CNN:       {LONG_MODEL_DIR / 'LINK_direction_model.pt'}")
    logger.info(f"  LONG scaler:    {LONG_MODEL_DIR / 'feature_scaler.joblib'}")
    logger.info(f"  SHORT CNN:      {SHORT_MODEL_DIR / 'LINK_short_model.pt'}")
    ...
    logger.info(f"  Meta LONG:      {META_MODEL_DIR / 'LINK_meta_long.joblib'}")
    logger.info(f"  Meta SHORT:     {META_MODEL_DIR / 'LINK_meta_short.joblib'}")
    ...
    logger.info(f"  SHORT architecture: DeepCNNShortModel (LayerNorm, stable for LINK)")
