...
def train():
    logger.info(f"\n{'='*70}")
    logger.info(f"ETH 1D-CNN TRAINING (Multi-scale Conv + Attention)")
    logger.info(f"{'='*70}\n")
    ...
    for epoch in range(EPOCHS):
        ...
        if is_better:
            ...
            torch.save({
                ...
                'model_type': 'cnn'
            }, MODEL_DIR / 'ETH_direction_model.pt')
    ...
    # Load best
    ckpt = torch.load(MODEL_DIR / 'ETH_direction_model.pt')
    ...
    torch.save(ckpt, MODEL_DIR / 'ETH_direction_model.pt')
    ...
    # Clean old files
    for old in ['ETH_direction_model_legacy.pt', 'eth_xgboost.joblib']:
        p = MODEL_DIR / old
        if p.exists():
            p.unlink()

    logger.info(f"\nModel size: {(MODEL_DIR / 'ETH_direction_model.pt').stat().st_size / 1024:.0f} KB")
