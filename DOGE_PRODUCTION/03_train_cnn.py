"""
DOGE 1D-CNN Training Script (V3)
===========================
Uses CNNDirectionModel - multi-scale convolutions + temporal attention.
V3 features: ATR-adaptive labeling, sample weighting, temperature calibration.
...
def train():
    logger.info(f"\n{'='*70}")
    logger.info(f"DOGE 1D-CNN TRAINING V3 (Multi-scale Conv + Attention + Temperature Calibration)")
    logger.info(f"{'='*70}\n")
    ...
    for epoch in range(EPOCHS):
        ...
        if is_better:
            ...
            torch.save({
                ...
                'model_type': 'cnn'
            }, MODEL_DIR / 'DOGE_direction_model.pt')
    ...
    # Load best
    ckpt = torch.load(MODEL_DIR / 'DOGE_direction_model.pt')
    ...
    torch.save(ckpt, MODEL_DIR / 'DOGE_direction_model.pt')
    ...
    logger.info(f"\nModel size: {(MODEL_DIR / 'DOGE_direction_model.pt').stat().st_size / 1024:.0f} KB")
