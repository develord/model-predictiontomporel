...
from direction_prediction_model import DirectionPredictionModel, LightweightDirectionModel, CNNDirectionModel
...
TRADING_FEE = 0.001
SLIPPAGE = 0.0005
CONFIDENCE_THRESHOLD = 0.60  # Optimized: 75% WR sweet spot

# Intelligent filters
MAX_CONSECUTIVE_LOSSES = 2
COOLDOWN_DAYS = 5
MIN_MOMENTUM_ALIGNMENT = 1  # At least 1/3 TFs bullish (relaxed for bear market)
MAX_VOLATILITY_REGIME = 2.5
USE_DYNAMIC_TP_SL = True  # Adjust TP/SL based on ATR
...
def load_model():
    ...
    if model_type == 'cnn':
        ...
        model = LightweightDirectionModel(feature_dim=feature_dim, sequence_length=seq_len,
                                           hidden_dim=164, num_layers=2, dropout=0.25)
    else:
...
def get_dynamic_tp_sl(row, entry_price):
    ...
    if atr is not None and pd.notna(atr) and atr > 0:
        # TP = 1x ATR, SL = 0.5x ATR (2:1 RR ratio)
        tp_mult = min(max(atr / entry_price, 0.008), 0.03)  # Clamp 0.8%-3%
        sl_mult = tp_mult * 0.5
...
def check_signal_filters(row, df_features, idx, consecutive_losses, cooldown_until, current_date, confidence=0.5):
    ...
    # 5. Macro trend filter - don't go LONG if price below SMA50
...
def backtest():
    ...
    logger.info(f"\n{'='*70}")
    logger.info(f"BTC BACKTEST - REALISTIC + INTELLIGENT FILTERS (Q1 2026)")
    logger.info(f"{'='*70}\n")
    ...
    for i in range(len(df_test)):
        ...
        logger.info(f"  {current_date.date()} | Conf:{c:.1%} | {result['exit_type']} @ ${result['exit_price']:.0f} | "
