...
def train():
    logger.info(f"\n{'='*70}")
    logger.info(f"AVAX SHORT CNN V2 TRAINING (DeepCNN + Bear Features)")
    logger.info(f"{'='*70}\n")
    ...
    # Final eval
    ckpt = torch.load(MODEL_DIR / 'AVAX_short_model.pt')
    ...
    ckpt_data = torch.load(MODEL_DIR / 'AVAX_short_model.pt')
    ckpt_data['temperature'] = optimal_temp
    torch.save(ckpt_data, MODEL_DIR / 'AVAX_short_model.pt')
    ...
    logger.info(f"\nModel: {(MODEL_DIR / 'AVAX_short_model.pt').stat().st_size / 1024:.0f} KB")
