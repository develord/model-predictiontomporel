"""
ADA SHORT CNN Training Script V3
==================================
Improved SHORT model with:
- DeepCNNShortModel (conv 3,5,9 + 3 blocks, seq=45)
- ATR-adaptive labeling (symmetric TP/SL = 1.5x ATR)
...
def train():
    logger.info(f"\n{'='*70}")
    logger.info(f"ADA SHORT CNN V2 TRAINING (DeepCNN + Bear Features)")
    logger.info(f"{'='*70}\n")
    ...
    # Final eval
    ckpt = torch.load(MODEL_DIR / 'ADA_short_model.pt')
    ...
    ckpt_data = torch.load(MODEL_DIR / 'ADA_short_model.pt')
    ckpt_data['temperature'] = optimal_temp
    torch.save(ckpt_data, MODEL_DIR / 'ADA_short_model.pt')

    # Calibrated confidence analysis
    all_confs2, all_dirs2, all_trues2 = [], [], []
    with torch.no_grad():
        for xb, yb in val_loader:
            logits = model(xb.to(device))
            calibrated_probs = torch.softmax(logits / optimal_temp, dim=1)
            confidence, direction = torch.max(calibrated_probs, dim=1)
            all_dirs2.extend(direction.cpu().numpy())
            all_confs2.extend(confidence.cpu().numpy())
            all_trues2.extend(yb.numpy())

    ad, ac, at = np.array(all_dirs2), np.array(all_confs2), np.array(all_trues2)
    ...
    logger.info(f"\nModel: {(MODEL_DIR / 'ADA_short_model.pt').stat().st_size / 1024:.0f} KB")
