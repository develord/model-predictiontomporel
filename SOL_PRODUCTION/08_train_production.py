"""
SOL Production Retraining Script
==================================
Retrains SOL models on the FULL dataset (no train/val split) for
...
LONG_TEMPERATURE = 2.109   # From SOL calibrated run (2.108952)
...
SHORT_TEMPERATURE = 2.157  # From SOL calibrated run (2.157389)
...
def train_long_cnn():
    ...
    }, LONG_MODEL_DIR / 'SOL_direction_model.pt')

    size_kb = (LONG_MODEL_DIR / 'SOL_direction_model.pt').stat().st_size / 1024
    logger.info(f"Saved: {LONG_MODEL_DIR / 'SOL_direction_model.pt'} ({size_kb:.0f} KB)")
...
def train_short_cnn():
    ...
    # Model - DeepCNNShortModel for SOL SHORT
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    }, SHORT_MODEL_DIR / 'SOL_short_model.pt')

    # Save feature list
    with open(SHORT_MODEL_DIR / 'short_features.json', 'w') as f:
        json.dump(feature_cols, f)

    size_kb = (SHORT_MODEL_DIR / 'SOL_short_model.pt').stat().st_size / 1024
    logger.info(f"Saved: {SHORT_MODEL_DIR / 'SOL_short_model.pt'} ({size_kb:.0f} KB)")
...
def train_meta(long_model, short_model):
    ...
    if len(lt) > 50:
        ...
        joblib.dump(xgb_long, META_MODEL_DIR / 'SOL_meta_long.joblib')
        size_kb = (META_MODEL_DIR / 'SOL_meta_long.joblib').stat().st_size / 1024
        logger.info(f"  Saved: SOL_meta_long.joblib ({size_kb:.0f} KB)")
    else:
    ...
    if len(st) > 50:
        ...
        joblib.dump(xgb_short, META_MODEL_DIR / 'SOL_meta_short.joblib')
        size_kb = (META_MODEL_DIR / 'SOL_meta_short.joblib').stat().st_size / 1024
        logger.info(f"  Saved: SOL_meta_short.joblib ({size_kb:.0f} KB)")
    else:
...
def main():
    logger.info(f"\n{'#'*70}")
    logger.info(f"#  SOL PRODUCTION RETRAIN - CNN + META MODELS ON FULL DATASET")
    ...
    logger.info(f"{'#'*70}")
    logger.info(f"  LONG CNN:       {LONG_MODEL_DIR / 'SOL_direction_model.pt'}")
    logger.info(f"  LONG scaler:    {LONG_MODEL_DIR / 'feature_scaler.joblib'}")
    logger.info(f"  SHORT CNN:      {SHORT_MODEL_DIR / 'SOL_short_model.pt'}")
    ...
    logger.info(f"  Meta LONG:      {META_MODEL_DIR / 'SOL_meta_long.joblib'}")
    logger.info(f"  Meta SHORT:     {META_MODEL_DIR / 'SOL_meta_short.joblib'}")
