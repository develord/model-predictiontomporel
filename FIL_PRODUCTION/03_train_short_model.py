"""
FIL SHORT CNN Training Script V3
==================================
...
SEQUENCE_LENGTH = 30  # Longer for SHORT patterns
BATCH_SIZE = 32
EPOCHS = 250
LEARNING_RATE = 0.001
PATIENCE = 60
...
def train():
    logger.info(f"\n{'='*70}")
    logger.info(f"FIL SHORT CNN V3 TRAINING (DeepCNN + Bear Features)")
    logger.info(f"{'='*70}\n")
    ...
    logger.info(f"Model: CNNDirectionModel (seq={SEQUENCE_LENGTH}) | Params: {n_params:,} | Device: {device}")
    ...
    # Final eval
    ckpt = torch.load(MODEL_DIR / 'FIL_short_model.pt')
    ...
    ckpt_data = torch.load(MODEL_DIR / 'FIL_short_model.pt')
    ckpt_data['temperature'] = optimal_temp
    torch.save(ckpt_data, MODEL_DIR / 'FIL_short_model.pt')
    ...
    logger.info(f"\nModel: {(MODEL_DIR / 'FIL_short_model.pt').stat().st_size / 1024:.0f} KB")
