...
def train():
    logger.info(f"\n{'='*70}")
    logger.info(f"BTC 1D-CNN TRAINING (Multi-scale Conv + Attention)")
    logger.info(f"{'='*70}\n")
    ...
    # Clean old files
    for old in ['BTC_direction_model_legacy.pt', 'btc_xgboost.joblib']:
        p = MODEL_DIR / old
        if p.exists():
            p.unlink()

    logger.info(f"\nModel size: {(MODEL_DIR / 'BTC_direction_model.pt').stat().st_size / 1024:.0f} KB")
