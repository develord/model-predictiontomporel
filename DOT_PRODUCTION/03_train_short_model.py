"""
DOT SHORT CNN Training Script V3
==================================
...
SEQUENCE_LENGTH = 30  # Reduced for smaller dataset
BATCH_SIZE = 32
EPOCHS = 300
LEARNING_RATE = 0.001
PATIENCE = 60
...
def train():
    logger.info(f"\n{'='*70}")
    logger.info(f"DOT SHORT CNN V3 TRAINING (DeepCNN + Bear Features)")
    logger.info(f"{'='*70}\n")
    ...
    # Class weights - boost SHORT class more aggressively
    w0 = len(train_labels) / (2 * n_0) if n_0 > 0 else 1.0
    w1 = len(train_labels) / (2 * n_1) * 1.5 if n_1 > 0 else 1.0  # 1.5x boost for SHORT
    ...
    # Model - Use simpler CNNDirectionModel for DOT (smaller dataset)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    from direction_prediction_model import CNNDirectionModel
    model = CNNDirectionModel(feature_dim=FEATURE_DIM, sequence_length=SEQUENCE_LENGTH, dropout=0.3).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    logger.info(f"Model: CNNDirectionModel-SHORT (seq={SEQUENCE_LENGTH}) | Params: {n_params:,} | Device: {device}")
    ...
    for epoch in range(EPOCHS):
        ...
        if acc > best_acc and n_short >= 1 and n_no >= 5:
            best_acc = acc
            best_epoch = epoch + 1
            patience_cnt = 0
            torch.save({
    ...
    # Fallback: if no checkpoint was saved, save current model
    if not (MODEL_DIR / 'DOT_short_model.pt').exists():
        logger.info("No checkpoint saved during training - saving final model as fallback")
        torch.save({
            'model_state_dict': model.state_dict(),
            'feature_dim': FEATURE_DIM,
            'sequence_length': SEQUENCE_LENGTH,
    ...
    # Final eval
    ckpt = torch.load(MODEL_DIR / 'DOT_short_model.pt')
    ...
    ckpt_data = torch.load(MODEL_DIR / 'DOT_short_model.pt')
    ckpt_data['temperature'] = optimal_temp
    torch.save(ckpt_data, MODEL_DIR / 'DOT_short_model.pt')
    ...
    logger.info(f"\nModel: {(MODEL_DIR / 'DOT_short_model.pt').stat().st_size / 1024:.0f} KB")
