"""
DOGE Meta Threshold Optimizer (V3)
=====================================
Grid search on CNN + Meta thresholds to maximize win rate.
Backtest on Q1 2026 (15min resolution).
...
def precompute_trade_outcomes(predictions, df_15m, df_test,
    ...
    for i in range(len(df_test)):
        ...
        tp_s, sl_s = get_tp_sl(row, entry_short, 'SHORT')
...
def optimize():
    logger.info(f"\n{'='*70}")
    logger.info(f"DOGE META THRESHOLD OPTIMIZER (V3)")
    logger.info(f"{'='*70}\n")

    # Load everything
    long_model, long_seq, long_ckpt = load_cnn(LONG_MODEL_DIR, 'DOGE_direction_model.pt')
    short_model, short_seq, short_ckpt = load_cnn(SHORT_MODEL_DIR, 'DOGE_short_model.pt')
    long_temp = long_ckpt.get('temperature', 1.0)
    short_temp = short_ckpt.get('temperature', 1.0)

    meta_long = joblib.load(META_MODEL_DIR / 'DOGE_meta_long.joblib')
    meta_short = joblib.load(META_MODEL_DIR / 'DOGE_meta_short.joblib')
    ...
    # Load 15min
    cache = DATA_DIR / 'doge_15min_q1_2026.csv'
    df_15m = pd.read_csv(cache, parse_dates=['timestamp'])
    df_15m.set_index('timestamp', inplace=True)

    logger.info(f"Test: {len(df_test)} days | 15min: {len(df_15m)} candles")
    logger.info(f"LONG seq={long_seq}, SHORT seq={short_seq}")
    logger.info(f"Temp: LONG={long_temp:.3f}, SHORT={short_temp:.3f}")
    ...
    long_meta_range = [0.0, 0.40, 0.45, 0.50, 0.52, 0.55]
