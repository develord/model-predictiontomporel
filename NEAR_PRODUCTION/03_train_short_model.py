"""
NEAR SHORT CNN Training Script V3
==================================
SHORT model with DeepCNNShortModel:
- Conv kernels 3,5,9 + 3 blocks
- Sequence length = 45
- ATR-symmetric TP/SL labels
- Bear-specific features (+15)
...
class DeepCNNShortModel(nn.Module):
    ...
    Conv kernels 3,5,9 + 3 blocks, seq=45"""
...
def train():
    logger.info(f"\n{'='*70}")
    logger.info(f"NEAR SHORT CNN V3 TRAINING (DeepCNN + Bear Features)")
    logger.info(f"{'='*70}\n")
    ...
    # Final eval
    ckpt = torch.load(MODEL_DIR / 'NEAR_short_model.pt')
    ...
    ckpt_data = torch.load(MODEL_DIR / 'NEAR_short_model.pt')
    ckpt_data['temperature'] = optimal_temp
    torch.save(ckpt_data, MODEL_DIR / 'NEAR_short_model.pt')
    ...
    logger.info(f"\nModel: {(MODEL_DIR / 'NEAR_short_model.pt').stat().st_size / 1024:.0f} KB")
