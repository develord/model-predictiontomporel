...
LONG_CONF = 0.65
SHORT_CONF = 0.55  # Allow SHORT signals
...
def fetch_15min():
    ...
    full = DATA_DIR / 'ada_15m_data.csv'
    if full.exists():
        df = pd.read_csv(full)
    ...
    cur = s
    while cur < e:
        c = ex.fetch_ohlcv('ADA/USDT', '15m', since=cur, limit=1000)
        if not c: break
        candles.extend(c)
        cur = c[-1][0] + 1
...
def backtest():
    logger.info(f"\n{'='*70}")
    logger.info(f"ADA INDEPENDENT LONG + SHORT BACKTEST (Q1 2026)")
    logger.info(f"{'='*70}\n")

    # Load models
    long_model, long_seq, long_ckpt = load_model(LONG_MODEL_DIR, 'ada_cnn_model.pt')
    short_model, short_seq, short_ckpt = load_model(SHORT_MODEL_DIR, 'ADA_short_model.pt')
    if not long_model or not short_model:
        logger.error("Missing model(s)")
        return

    # LONG features
    with open(BASE_DIR / 'models/ada_cnn_features.json') as f:
    ...
    if trades:
        ...
        tdf.to_csv(RESULTS_DIR / 'ada_independent_backtest.csv', index=False)
    else:
