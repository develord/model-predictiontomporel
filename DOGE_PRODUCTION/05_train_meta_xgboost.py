...
def load_cnn_model(model_dir, model_file):
    ...
    is_deep = model_type == 'deep_cnn' or any('conv3_1' in k or 'conv9_1' in k for k in ckpt['model_state_dict'].keys())
    if is_deep:
...
def train_meta():
    logger.info(f"\n{'='*70}")
    logger.info(f"DOGE META-MODEL XGBOOST TRAINING (V3)")
    logger.info(f"{'='*70}\n")

    # Load CNN models
    long_model, long_seq, long_ckpt = load_cnn_model(LONG_MODEL_DIR, 'DOGE_direction_model.pt')
    short_model, short_seq, short_ckpt = load_cnn_model(SHORT_MODEL_DIR, 'DOGE_short_model.pt')
    ...
    if len(lt) > 50:
        ...
        # Save
        joblib.dump(xgb_long, META_MODEL_DIR / 'DOGE_meta_long.joblib')
        logger.info(f"\n  Saved: DOGE_meta_long.joblib")
    else:
    ...
    if len(st) > 50:
        ...
        joblib.dump(xgb_short, META_MODEL_DIR / 'DOGE_meta_short.joblib')
        logger.info(f"\n  Saved: DOGE_meta_short.joblib")
    else:
