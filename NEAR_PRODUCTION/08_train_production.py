"""
NEAR Production Retraining Script
==================================
Retrains NEAR models on the FULL dataset (no train/val split) for
production deployment:
  1. LONG CNN  (CNNDirectionModel)
  2. SHORT CNN (DeepCNNShortModel)
  3. META XGBoost (LONG + SHORT meta-models)
...
Key principles:
  ...
  - Meta models retrained on full-data CNN predictions
...
# LONG CNN params
# ============================================================
LONG_FIXED_EPOCHS = 4      # Best epoch from calibrated run (epoch=3, 0-indexed)
LONG_BATCH_SIZE = 64
LONG_LR = 0.0015
LONG_NOISE_STD = 0.015
LONG_LABEL_SMOOTHING = 0.1
LONG_AUGMENT_COPIES = 2    # 3x total (original + 2 copies)
LONG_TEMPERATURE = 2.2225  # From NEAR calibrated run

# ============================================================
# SHORT CNN params (DeepCNNShortModel, seq=45)
# ============================================================
SHORT_SEQUENCE_LENGTH = 45
SHORT_FIXED_EPOCHS = 8     # Increased from best_epoch=2 (too few for stable convergence)
SHORT_BATCH_SIZE = 64
SHORT_LR = 0.0005
SHORT_NOISE_STD = 0.01
SHORT_LABEL_SMOOTHING = 0.1
SHORT_AUGMENT_COPIES = 3   # 4x total
SHORT_TEMPERATURE = 2.1124 # From NEAR calibrated run
...
def train_long_cnn():
    ...
    df = pd.read_csv(DATA_DIR / 'near_features.csv')
    ...
    # Augmentation (3x total) - clip after noise to prevent out-of-range values
    aug_seqs, aug_labels, aug_weights = [train_seqs], [train_labels], [train_weights]
    for _ in range(LONG_AUGMENT_COPIES):
        noisy = train_seqs + np.random.normal(0, LONG_NOISE_STD, train_seqs.shape).astype(np.float32)
        aug_seqs.append(np.clip(noisy, -5, 5))
    ...
    }, LONG_MODEL_DIR / f'{CRYPTO}_direction_model.pt')

    size_kb = (LONG_MODEL_DIR / f'{CRYPTO}_direction_model.pt').stat().st_size / 1024
    logger.info(f"Saved: {LONG_MODEL_DIR / f'{CRYPTO}_direction_model.pt'} ({size_kb:.0f} KB)")
...
def train_short_cnn():
    ...
    # Augment (4x total) - clip after noise to prevent out-of-range values
    aug_seqs, aug_labels = [train_seqs], [train_labels]
    for _ in range(SHORT_AUGMENT_COPIES):
        noisy = train_seqs + np.random.normal(0, SHORT_NOISE_STD, train_seqs.shape).astype(np.float32)
        aug_seqs.append(np.clip(noisy, -5, 5))
        aug_labels.append(train_labels)
    train_seqs_aug = np.concatenate(aug_seqs)
    train_labels_aug = np.concatenate(aug_labels)
    ...
    # Model - DeepCNNShortModel for NEAR SHORT
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    }, SHORT_MODEL_DIR / f'{CRYPTO}_short_model.pt')

    # Save feature list
    with open(SHORT_MODEL_DIR / 'short_features.json', 'w') as f:
        json.dump(feature_cols, f)

    size_kb = (SHORT_MODEL_DIR / f'{CRYPTO}_short_model.pt').stat().st_size / 1024
    logger.info(f"Saved: {SHORT_MODEL_DIR / f'{CRYPTO}_short_model.pt'} ({size_kb:.0f} KB)")
...
# ============================================================
# STEP 3: Retrain META XGBoost on full-data CNN predictions
# ============================================================
def train_meta_models():
    ...
    # Load retrained CNN models
    long_ckpt = torch.load(LONG_MODEL_DIR / f'{CRYPTO}_direction_model.pt', map_location='cpu', weights_only=False)
    ...
    long_model.eval()
    long_temp = long_ckpt['temperature']
    long_seq = long_ckpt['sequence_length']

    short_ckpt = torch.load(SHORT_MODEL_DIR / f'{CRYPTO}_short_model.pt', map_location='cpu', weights_only=False)
    ...
    short_model.eval()
    short_temp = short_ckpt['temperature']
    short_seq = short_ckpt['sequence_length']

    logger.info(f"Loaded retrained CNNs: LONG temp={long_temp:.3f}, SHORT temp={short_temp:.3f}")
    ...
    # Generate predictions + outcomes for ALL data
    logger.info("Generating CNN predictions on full dataset...")

    # Market context features for meta
    market_cols = [
    ...
    # === TRAIN LONG META on ALL data ===
    logger.info(f"\n--- LONG META (all data, no val split) ---")
    ...
    if len(lt) > 50:
        ...
        joblib.dump(xgb_long, META_MODEL_DIR / f'{CRYPTO}_meta_long.joblib')
        size_kb = (META_MODEL_DIR / f'{CRYPTO}_meta_long.joblib').stat().st_size / 1024
        logger.info(f"  Saved: {CRYPTO}_meta_long.joblib ({size_kb:.0f} KB)")

        # Quick self-check
        lt_pred = xgb_long.predict_proba(X_lt)[:, 1]
        for t in [0.45, 0.50, 0.55, 0.60]:
            mask = lt_pred >= t
            if mask.sum() > 0:
                wr = y_lt[mask].mean() * 100
                logger.info(f"    Train self-check >= {t:.0%}: {mask.sum()} signals, WR: {wr:.1f}%")
    else:
        logger.warning(f"  Not enough LONG meta samples ({len(lt)})")

    # === TRAIN SHORT META on ALL data ===
    logger.info(f"\n--- SHORT META (all data, no val split) ---")
    ...
    if len(st) > 50:
        ...
        joblib.dump(xgb_short, META_MODEL_DIR / f'{CRYPTO}_meta_short.joblib')
        size_kb = (META_MODEL_DIR / f'{CRYPTO}_meta_short.joblib').stat().st_size / 1024
        logger.info(f"  Saved: {CRYPTO}_meta_short.joblib ({size_kb:.0f} KB)")

        # Quick self-check
        st_pred = xgb_short.predict_proba(X_st)[:, 1]
        for t in [0.40, 0.45, 0.50, 0.55]:
            mask = st_pred >= t
            if mask.sum() > 0:
                wr = y_st[mask].mean() * 100
                logger.info(f"    Train self-check >= {t:.0%}: {mask.sum()} signals, WR: {wr:.1f}%")
    else:
        logger.warning(f"  Not enough SHORT meta samples ({len(st)})")
...
def main():
    logger.info(f"\n{'#'*70}")
    logger.info(f"#  NEAR PRODUCTION RETRAIN - CNN + META MODELS ON FULL DATASET")
    logger.info(f"#  No train/val split | Fixed epochs | Hardcoded temperatures")
    logger.info(f"#  Optimizer targets: LCNN>=75% SCNN>=50% LMeta>=45% SMeta>=40%")
    logger.info(f"{'#'*70}\n")
    ...
    # Step 3: META XGBoost (NEAR has meta models)
    train_meta_models()

    logger.info(f"\n{'#'*70}")
    logger.info(f"#  PRODUCTION RETRAIN COMPLETE")
    logger.info(f"{'#'*70}")
    logger.info(f"  LONG CNN:       {LONG_MODEL_DIR / f'{CRYPTO}_direction_model.pt'}")
    logger.info(f"  LONG scaler:    {LONG_MODEL_DIR / 'feature_scaler.joblib'}")
    logger.info(f"  SHORT CNN:      {SHORT_MODEL_DIR / f'{CRYPTO}_short_model.pt'}")
    ...
    logger.info(f"  META LONG:      {META_MODEL_DIR / f'{CRYPTO}_meta_long.joblib'}")
    logger.info(f"  META SHORT:     {META_MODEL_DIR / f'{CRYPTO}_meta_short.joblib'}")
