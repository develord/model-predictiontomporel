...
# Import DeepCNNShortModel from 03_train_short_model
sys.path.insert(0, str(BASE_DIR))
from importlib import import_module
_short_module = import_module('03_train_short_model')
DeepCNNShortModel = _short_module.DeepCNNShortModel
...
def load_cnn(model_dir, model_file):
    ...
    if is_deep:
        model = DeepCNNShortModel(feature_dim=feat_dim, sequence_length=seq_len, dropout=0.30)
    else:
...
def run_backtest_fast(outcomes, long_cnn_th, short_cnn_th, long_meta_th, short_meta_th,
    ...
    """Fast backtest using pre-computed outcomes (no sim_trade calls)."""
    capital = 1000.0
    trades = []
        ...
        if o['l_dir'] == 1 and o['l_conf'] >= long_cnn_th:
            if long_cool and date <= long_cool:
                ...
                if take:
                    ...
                    fee = pos_val * TRADING_FEE * 2  # entry + exit
...
def optimize():
    logger.info(f"\n{'='*70}")
    logger.info(f"{CRYPTO} META THRESHOLD OPTIMIZER")
    logger.info(f"{'='*70}\n")

    # Load everything
    long_model, long_seq, long_ckpt = load_cnn(LONG_MODEL_DIR, f'{CRYPTO}_direction_model.pt')
    short_model, short_seq, short_ckpt = load_cnn(SHORT_MODEL_DIR, f'{CRYPTO}_short_model.pt')
    long_temp = long_ckpt.get('temperature', 1.0)
    short_temp = short_ckpt.get('temperature', 1.0)

    meta_long = joblib.load(META_MODEL_DIR / f'{CRYPTO}_meta_long.joblib')
    meta_short = joblib.load(META_MODEL_DIR / f'{CRYPTO}_meta_short.joblib')
    ...
    df = pd.read_csv(DATA_DIR / f'{CRYPTO.lower()}_features.csv')
    df['date'] = pd.to_datetime(df['date'])

    # Add bear features for SHORT
    try:
        df = _short_module.add_bear_features(df)
    except:
        pass

    df_wide = df[df['date'] >= '2025-01-01'].copy()

    long_raw = np.clip(df_wide[long_feature_cols].fillna(0).replace([np.inf, -np.inf], 0).values.astype(np.float32), -1e6, 1e6)
    ...
    for c in short_feature_cols:
        if c not in df_wide.columns:
            df_wide[c] = 0
    short_raw = np.clip(df_wide[short_feature_cols].fillna(0).replace([np.inf, -np.inf], 0).values.astype(np.float32), -1e6, 1e6)
    ...
    # Load 15min data
    df_15m = pd.read_csv(DATA_DIR / f'{CRYPTO.lower()}_15m_data.csv')
    df_15m['date'] = pd.to_datetime(df_15m['date'])
    df_15m.set_index('date', inplace=True)
    # Filter to backtest period
    df_15m = df_15m[df_15m.index >= '2025-12-01']  # Keep some buffer before backtest start
    ...
    # Pre-compute all trade outcomes ONCE (expensive sim_trade calls)
    ...
    # Grid search (now fast - no sim_trade calls)
                ...
                for sm in short_meta_range:
                    for cd in cooldown_range:
                        for mc in consec_range:
                            ...
                            # Only count TP/SL trades for WR (EOD excluded)
    ...
    # Filter: at least 5 real trades
    rdf_valid = rdf[rdf['tp_sl_trades'] >= 5].copy()

    # Sort by WR, then by return
