"""
XRP Production Retraining Script
==================================
Retrains XRP models on the FULL dataset (no train/val split) for
production deployment:
  1. LONG CNN  (CNNDirectionModel)
  2. SHORT CNN (DeepCNNShortModel)
  3. Meta XGBoost LONG only (SMeta = NoMeta per optimizer)
...
# ============================================================
# DeepCNNShortModel (inline, same architecture as training)
# ============================================================
class DeepCNNShortModel(nn.Module):
    ...
    def __init__(self, feature_dim, sequence_length=45, dropout=0.35):
        ...
        # Feature projection (higher eps for BatchNorm stability on constant features)
        self.input_proj = nn.Sequential(
            nn.Linear(feature_dim, 96),
            nn.BatchNorm1d(sequence_length, eps=1e-3),
            nn.GELU(),
            nn.Dropout(dropout)
        )
        ...
        self.bn1 = nn.BatchNorm1d(144, eps=1e-3)
...
# ============================================================
# LONG CNN params (from 03_train_cnn.py calibrated run)
# ============================================================
LONG_FIXED_EPOCHS = 24     # Best epoch from calibrated run
LONG_BATCH_SIZE = 64
LONG_LR = 0.0015
LONG_NOISE_STD = 0.015
LONG_LABEL_SMOOTHING = 0.1
LONG_AUGMENT_COPIES = 2    # 3x total (original + 2 copies)
LONG_TEMPERATURE = 1.7843  # From XRP calibrated run

# ============================================================
# SHORT CNN params (from 03_train_short_model.py calibrated run)
# Production uses lower LR + higher dropout for full-data stability
# (original LR=0.002/dropout=0.15 causes NaN on full data without early stopping)
# ============================================================
SHORT_FIXED_EPOCHS = 31    # Best epoch from calibrated run
SHORT_BATCH_SIZE = 64
SHORT_LR = 0.0005          # Reduced from 0.002 for full-data stability
SHORT_DROPOUT = 0.30       # Increased from 0.15 for full-data stability
SHORT_NOISE_STD = 0.01
SHORT_LABEL_SMOOTHING = 0.05
SHORT_AUGMENT_COPIES = 2   # 3x total (original + 2 copies)
SHORT_TEMPERATURE = 1.1272 # From XRP calibrated run

# ============================================================
# Meta XGBoost params
# ============================================================
META_N_ESTIMATORS = 300    # Fixed, no early stopping
...
def train_epoch_simple(model, dataloader, criterion, optimizer, device, grad_clip):
    """Train one epoch without sample weights. NaN-safe for production stability."""
    model.train()
    total_loss, correct, total = 0, 0, 0
    nan_batches = 0
    for X_b, y_b in dataloader:
        ...
        optimizer.zero_grad()
        logits = model(X_b)
        # Skip batch if forward produced NaN (corrupted BatchNorm state)
        ...
        if torch.isnan(loss) or torch.isinf(loss):
            ...
            continue
        loss.backward()
        # Replace NaN/inf gradients with 0 before clipping
        ...
        correct += (pred == y_b).sum().item()
        total += y_b.size(0)
    valid = max(len(dataloader) - nan_batches, 1)
    if nan_batches > 0:
        logger.warning(f"  NaN batches skipped: {nan_batches}/{len(dataloader)}")
    return total_loss / valid, correct / max(total, 1)


# ============================================================
# Meta feature builder (matches 05_train_meta_xgboost.py)
# ============================================================
...
def simulate_short_outcome(df, i, atr_val):
    ...
    entry = df.iloc[i]['close']
    if pd.notna(atr_val) and atr_val > 0:
        tp = entry - atr_val * SHORT_ATR_TP_MULT
        sl = entry + atr_val * SHORT_ATR_SL_MULT
    else:
        tp = entry * (1 - SHORT_FIXED_TP_PCT)
        sl = entry * (1 + SHORT_FIXED_SL_PCT)
    for j in range(i + 1, min(i + 1 + SHORT_BASE_LOOKAHEAD, len(df))):
        if df.iloc[j]['low'] <= tp:
            return 1
        if df.iloc[j]['high'] >= sl:
            return 0
...
def train_long_cnn():
    ...
    df = pd.read_csv(DATA_DIR / 'xrp_features.csv')
    ...
    for i in range(LONG_SEQUENCE_LENGTH, len(X_scaled)):
        if y_raw[i] != -1:
            train_seqs.append(X_scaled[i - LONG_SEQUENCE_LENGTH:i])
    ...
    model = CNNDirectionModel(feature_dim=FEATURE_DIM, sequence_length=LONG_SEQUENCE_LENGTH, dropout=0.4).to(device)
    ...
    for epoch in range(LONG_FIXED_EPOCHS):
        train_loss, train_acc = train_epoch_weighted(
            model, train_loader, criterion, optimizer, device, LONG_GRAD_CLIP
        )
    ...
    }, LONG_MODEL_DIR / 'XRP_direction_model.pt')

    size_kb = (LONG_MODEL_DIR / 'XRP_direction_model.pt').stat().st_size / 1024
    logger.info(f"Saved: {LONG_MODEL_DIR / 'XRP_direction_model.pt'} ({size_kb:.0f} KB)")
...
def train_short_cnn():
    ...
    # Scale ALL data (extra careful inf/nan handling for bear features)
    ...
    logger.info(f"Scaled data check - NaN: {np.isnan(X_scaled).sum()}, Inf: {np.isinf(X_scaled).sum()}")
    joblib.dump(scaler, SHORT_MODEL_DIR / 'feature_scaler_short.joblib')

    # Build sequences on ALL data (skip early rows where weekly features are incomplete)
    y_raw = df['label'].values
    # Skip first 100 rows to ensure all timeframe features are populated
    min_start = max(SHORT_SEQUENCE_LENGTH, 100)
    seqs, labels = [], []
    for i in range(min_start, len(X_scaled)):
        if y_raw[i] != -1:
            seq = X_scaled[i - SHORT_SEQUENCE_LENGTH:i]
            # Skip sequences with too many constant columns (numerical instability)
            n_const = sum(1 for f in range(seq.shape[1]) if seq[:, f].std() == 0)
            if n_const > seq.shape[1] * 0.3:  # Skip if >30% features are constant
                continue
            seqs.append(seq)
    ...
    # Class weights - 2x boost for SHORT class (matches training script)
    w0 = len(train_labels) / (2 * n_0) if n_0 > 0 else 1.0
    w1 = (len(train_labels) / (2 * n_1)) * 2.0 if n_1 > 0 else 2.0
    ...
    check_tensor = torch.FloatTensor(train_seqs_aug)
    logger.info(f"Data integrity: NaN={torch.isnan(check_tensor).any().item()}, "
                f"Inf={torch.isinf(check_tensor).any().item()}, "
                f"Max={check_tensor.abs().max().item():.3f}")

    # Model - DeepCNNShortModel with stabilized dropout for production
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = DeepCNNShortModel(feature_dim=FEATURE_DIM, sequence_length=SHORT_SEQUENCE_LENGTH, dropout=SHORT_DROPOUT).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    logger.info(f"Model: DeepCNNShortModel (SHORT) | Params: {n_params:,} | Device: {device}")
    logger.info(f"Class weights: w0={w0:.4f}, w1={w1:.4f}")
    ...
    for epoch in range(SHORT_FIXED_EPOCHS):
        train_loss, train_acc = train_epoch_simple(
            model, train_loader, criterion, optimizer, device, SHORT_GRAD_CLIP
        )
        scheduler.step(epoch)
    ...
    }, SHORT_MODEL_DIR / 'XRP_short_model.pt')

    # Save feature list
    with open(SHORT_MODEL_DIR / 'short_features.json', 'w') as f:
        json.dump(feature_cols, f)

    size_kb = (SHORT_MODEL_DIR / 'XRP_short_model.pt').stat().st_size / 1024
    logger.info(f"Saved: {SHORT_MODEL_DIR / 'XRP_short_model.pt'} ({size_kb:.0f} KB)")
...
# ============================================================
# STEP 3: Train Meta XGBoost (LONG only, SMeta = NoMeta)
# ============================================================
def train_meta_xgboost():
    logger.info(f"\n{'='*70}")
    logger.info(f"STEP 3: META XGBOOST LONG - Production Retrain (ALL data, {META_N_ESTIMATORS} trees)")
    logger.info(f"{'='*70}\n")

    # Load the just-trained CNN models
    long_ckpt = torch.load(LONG_MODEL_DIR / 'XRP_direction_model.pt', map_location='cpu', weights_only=False)
    long_model = CNNDirectionModel(
        feature_dim=long_ckpt['feature_dim'],
        sequence_length=long_ckpt['sequence_length'],
        dropout=0.4
    )
    long_model.load_state_dict(long_ckpt['model_state_dict'])
    long_model.eval()
    long_temp = long_ckpt.get('temperature', LONG_TEMPERATURE)

    short_ckpt = torch.load(SHORT_MODEL_DIR / 'XRP_short_model.pt', map_location='cpu', weights_only=False)
    short_model = DeepCNNShortModel(
        feature_dim=short_ckpt['feature_dim'],
        sequence_length=short_ckpt['sequence_length'],
        dropout=SHORT_DROPOUT
    )
    short_model.load_state_dict(short_ckpt['model_state_dict'])
    short_model.eval()
    short_temp = short_ckpt.get('temperature', SHORT_TEMPERATURE)

    logger.info(f"Loaded LONG model (dim={long_ckpt['feature_dim']}, temp={long_temp:.3f})")
    logger.info(f"Loaded SHORT model (dim={short_ckpt['feature_dim']}, temp={short_temp:.3f})")
    ...
    # Load data and add bear features
    df = pd.read_csv(DATA_DIR / 'xrp_features.csv')
    df['date'] = pd.to_datetime(df['date'])
    df = add_bear_features(df)
    ...
    # Prepare scaled features for both CNNs
    ...
    long_seq = long_ckpt['sequence_length']
    short_seq = short_ckpt['sequence_length']
    seq = max(long_seq, short_seq)

    # Generate CNN predictions + outcomes for every day
    ...
    # === LONG META ONLY (SMeta = NoMeta per optimizer) ===
    logger.info(f"\n--- Training LONG Meta-Model ---")
    ...
    if len(lt) > 50:
        ...
        # No early stopping, no eval set -- train on ALL data
        xgb_long.fit(X_lt, y_lt)

        joblib.dump(xgb_long, META_MODEL_DIR / 'XRP_meta_long.joblib')
        logger.info(f"  Saved: {META_MODEL_DIR / 'XRP_meta_long.joblib'}")
        ...
        logger.warning(f"  Not enough LONG samples ({len(lt)}), skipping")

    logger.info(f"\n  SHORT Meta: SKIPPED (SMeta = NoMeta per optimizer)")

    # Save meta feature list
    with open(META_MODEL_DIR / 'meta_features.json', 'w') as f:
        json.dump(feature_cols_meta, f, indent=2)
    logger.info(f"\nMeta features ({len(feature_cols_meta)}) saved: {META_MODEL_DIR / 'meta_features.json'}")
...
def main():
    logger.info(f"\n{'#'*70}")
    logger.info(f"#  XRP PRODUCTION RETRAIN - CNN + META LONG ON FULL DATASET")
    ...
    # Step 3: Meta XGBoost LONG only (uses the just-trained CNN models)
    train_meta_xgboost()

    logger.info(f"\n{'#'*70}")
    logger.info(f"#  PRODUCTION RETRAIN COMPLETE")
    logger.info(f"{'#'*70}")
    logger.info(f"  LONG CNN:       {LONG_MODEL_DIR / 'XRP_direction_model.pt'}")
    logger.info(f"  LONG scaler:    {LONG_MODEL_DIR / 'feature_scaler.joblib'}")
    logger.info(f"  SHORT CNN:      {SHORT_MODEL_DIR / 'XRP_short_model.pt'}")
    ...
    logger.info(f"  Meta LONG:      {META_MODEL_DIR / 'XRP_meta_long.joblib'}")
    logger.info(f"  Meta SHORT:     SKIPPED (NoMeta)")
    ...
    logger.info(f"  Meta trees: {META_N_ESTIMATORS} (LONG only, no early stopping)")
