"""
DOT Feature Engineering Script - Multi-Timeframe + Non-Technical
================================================================
...
- Market regime features (126 features)
...
def create_cross_tf_features(df):
    ...
    if len(rsi_cols) >= 2:
        ...
        df['rsi_overdotd_count'] = sum((df[c] < 30).astype(int) for c in rsi_cols)
...
def build_features():
    ...
    logger.info(f"\n{'='*70}")
    logger.info(f"DOT MULTI-TF FEATURE ENGINEERING")
    logger.info(f"{'='*70}\n")
    ...
    # Save merged data
    output_file = DATA_DIR / 'dot_features.csv'
